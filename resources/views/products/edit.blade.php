    ...
    <div class="space-y-6">
        <div class="rounded-[32px] border border-white/10 bg-slate-900/80 p-6 shadow-2xl shadow-slate-950/40 backdrop-blur-lg">
            <div class="flex flex-col gap-2">
                <h2 class="text-3xl font-semibold tracking-tight text-white">Edit Barang</h2>
                <p class="text-sm text-slate-400">Perbarui informasi barang "{{ $product->nama_barang }}".</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
            <form method="POST" action="{{ route('products.update', $product) }}" enctype="multipart/form-data" class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <input id="nama_barang" name="nama_barang" type="text" value="{{ old('nama_barang', $product->nama_barang) }}" required autofocus
                ...
                <div>
                    ...
                    <input id="harga" name="harga" type="number" value="{{ old('harga', $product->harga) }}" required min="0" step="0.01"
                           class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-4 py-2 text-slate-900 focus:border-emerald-500 focus:ring-emerald-500" />
                ...
                <div>
                    ...
                    <input id="stok" name="stok" type="number" value="{{ old('stok', $product->stok) }}" required min="0"
                           class="mt-2 block w-full rounded-xl border border-slate-300 bg-white px-4 py-2 text-slate-900 focus:border-emerald-500 focus:ring-emerald-500" />
                ...
                @if($product->gambar)
                    <div>
                        <label class="block text-sm font-medium text-slate-900">Gambar Saat Ini</label>
                        <img src="{{ $product->gambar_url }}" alt="{{ $product->nama_barang }}" class="mt-2 h-32 w-32 rounded-xl object-cover border border-slate-200" onerror="this.src='https://via.placeholder.com/300'" />
                    </div>
                @endif

                <div>
                    <label for="gambar" class="block text-sm font-medium text-slate-900">Ganti Gambar (Opsional)</label>
                    ...
                    <p class="mt-2 text-sm text-slate-500">Biarkan kosong jika tidak ingin mengganti gambar. Format: JPG, PNG, GIF. Maksimal 2MB.</p>
                    @error('gambar') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>

                <div id="image-preview" class="hidden">
                    <label class="block text-sm font-medium text-slate-900">Preview Gambar Baru</label>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-slate-200">
                    ...
                    <button type="submit" class="inline-flex items-center px-4 py-2 rounded-xl bg-emerald-600 text-sm font-medium text-white hover:bg-emerald-700 transition">Perbarui Barang</button>
                </div>
            </form>
        </div>
    </div>
